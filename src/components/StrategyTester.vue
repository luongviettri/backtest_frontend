  ...
  <div class="relative w-full h-screen overflow-hidden">
      ...
      <div class="flex items-center space-x-4">
        ...
        />
          ...
          <div class="relative">
            ...
            <vue-feather type="chevron-down" size="16" class="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-slate-500"></vue-feather>
          </div>
          <!-- ===== KẾT THÚC THÊM MỚI ===== -->
    ...
    <aside id="indicator-toolbox" class="bg-white border-r border-slate-200 p-4 flex flex-col">
      <h2 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">
        THƯ VIỆN CHỈ BÁO ({{ availableIndicators.length }}/50)
      </h2>
      <div class="relative flex-grow overflow-y-auto pr-2 space-y-3">
        ...
        />
        <div class="relative opacity-60 cursor-not-allowed bg-slate-50 border border-slate-200 rounded-lg p-3 flex items-center space-x-3">
          <vue-feather type="zap" size="20" class="w-8 h-8 p-1.5 bg-amber-100 text-amber-600 rounded-md"></vue-feather>
          <div>
            <p class="font-semibold text-slate-800">Ichimoku Cloud</p>
            <p class="text-xs text-slate-500">Mây Ichimoku</p>
          </div>
          <div class="absolute inset-0 flex items-center justify-center">
            <vue-feather type="lock" size="16" class="w-5 h-5 text-amber-700 bg-amber-200 p-1 rounded-full"></vue-feather>
          </div>
        </div>
      </div>
    ...
    <component
      ...
      @delete-condition="
        strategyStore.deleteCondition($event.zoneType, $event.conditionId)
      "
      class="bg-slate-50"
    ...
    <div id="config-inspector-wrapper" class="bg-slate-50 border-l border-slate-200 p-4 space-y-4 overflow-y-auto min-h-0">
      ...
      <div v-else class="space-y-4">
        ...
        <div v-if="strategyStore.selectedCondition.role" class="bg-white border border-blue-300 rounded-lg p-4 space-y-4 shadow-sm">
          <div class="flex items-center space-x-2 pb-3 border-b"><vue-feather type="zap" class="text-blue-600" size="18"></vue-feather><h3 class="font-bold text-slate-800 text-base">Cấu hình Tín hiệu: {{ strategyStore.selectedCondition.type }}</h3></div>
          ...
          <div class="space-y-3 text-sm">
            ...
            <div v-if="strategyStore.selectedCondition.params.period !== undefined">
              ...
              <input type="number" v-model.number="strategyStore.selectedCondition.params.period" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div v-if="strategyStore.selectedCondition.timeframe">
              ...
              <select v-model="strategyStore.selectedCondition.timeframe" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
          ...
          <div class="space-y-3 text-sm pt-3 border-t">
            ...
            <div class="flex items-center space-x-2">
              <span class="font-mono text-slate-800">{{ strategyStore.selectedCondition.type }}</span>
              <select v-model="strategyStore.selectedCondition.longCondition.operator" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                <option value="<">Nhỏ hơn (&lt;)</option>
                <option value=">">Lớn hơn (&gt;)</option>
                <option value="cross_above">Cắt lên trên</option>
              </select>
              <input type="number" v-model.number="strategyStore.selectedCondition.longCondition.value" class="w-24 px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
            </div>
          </div>
          ...
          <div class="space-y-3 text-sm pt-3 border-t">
            ...
            <div class="flex items-center space-x-2">
              <span class="font-mono text-slate-800">{{ strategyStore.selectedCondition.type }}</span>
              <select v-model="strategyStore.selectedCondition.shortCondition.operator" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                <option value="<">Nhỏ hơn (&lt;)</option>
                <option value=">">Lớn hơn (&gt;)</option>
                <option value="cross_below">Cắt xuống dưới</option>
              </select>
              <input type="number" v-model.number="strategyStore.selectedCondition.shortCondition.value" class="w-24 px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
            </div>
          </div>
          ...
          <div class="space-y-3 text-sm pt-3 border-t">
            ...
            <div>
              <select v-model="strategyStore.selectedCondition.role" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
        ...
        <div v-else-if="strategyStore.selectedCondition.longCondition && strategyStore.selectedCondition.shortCondition && !strategyStore.selectedCondition.role" class="bg-white border border-purple-300 rounded-lg p-4 space-y-4 shadow-sm">
          <div class="flex items-center space-x-2 pb-3 border-b"><vue-feather type="filter" class="text-purple-600" size="18"></vue-feather><h3 class="font-bold text-slate-800 text-base">Cấu hình Bộ lọc: {{ strategyStore.selectedCondition.type }}</h3></div>
          ...
          <div class="space-y-3 text-sm pt-3 border-t">
            ...
            <div class="flex items-center space-x-2">
              ...
              <span class="font-mono text-slate-800">{{ strategyStore.selectedCondition.type }}({{ strategyStore.selectedCondition.params.period }})</span>
            </div>
          </div>
        ...
        <div v-else-if="strategyStore.selectedCondition.longCondition && !strategyStore.selectedCondition.role" class="bg-white border border-orange-300 rounded-lg p-4 space-y-4 shadow-sm">
          <div class="flex items-center space-x-2 pb-3 border-b"><vue-feather type="log-out" class="text-orange-600" size="18"></vue-feather><h3 class="font-bold text-slate-800 text-base">Cấu hình Thoát lệnh: {{ strategyStore.selectedCondition.type }}</h3></div>
        ...
        <div v-else-if="strategyStore.selectedCondition.condition" class="bg-white border border-purple-300 rounded-lg p-4 space-y-4 shadow-sm">
           <div class="flex items-center space-x-2 pb-3 border-b"><vue-feather type="filter" class="text-purple-600" size="18"></vue-feather><h3 class="font-bold text-slate-800 text-base">Cấu hình Bộ lọc: {{ strategyStore.selectedCondition.type }}</h3></div>
           <div class="space-y-3 text-sm">
              <!-- Tham số chung -->
              <div v-if="strategyStore.selectedCondition.params.period !== undefined">
              ...
              <div>
                ...
                <div class="flex items-center space-x-2">
                  <span class="font-mono text-slate-800">{{ strategyStore.selectedCondition.type }}</span>
                  <select v-model="strategyStore.selectedCondition.condition.operator" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                    <option value=">">Lớn hơn (&gt;)</option>
                    <option value="<">Nhỏ hơn (&lt;)</option>
                  </select>
                  <!-- Giá trị có thể là số hoặc một chuỗi như 'price' -->
                  <input :type="typeof strategyStore.selectedCondition.condition.value === 'number' ? 'number' : 'text'" v-model="strategyStore.selectedCondition.condition.value" class="w-24 px-3 py-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
              </div>
           </div>
        </div>
      ...
      <div class="bg-white border border-slate-200 rounded-lg p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label for="pair" class="block text-sm font-medium text-slate-700 mb-1">Cặp Giao dịch</label>
            <select id="pair" v-model="strategyStore.pair" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
              <option>BTC/USDT</option>
              <option>ETH/USDT</option>
            </select>
          </div>
          <div>
            <label for="timeframe" class="block text-sm font-medium text-slate-700 mb-1">Khung Thời gian</label>
            <select id="timeframe" v-model="strategyStore.timeframe" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
      ...
      <!-- 2. QUẢN LÝ RỦI RO (CƠ BẢN) -->
      <div class="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
          <h3 class="font-bold text-slate-800 text-base">Quản lý Rủi ro (Cơ bản)</h3>
          <div class="grid grid-cols-2 gap-3">
            <!-- Cắt lỗ -->
            <div>
              <div class="flex justify-between items-center mb-1">
                <label for="stop-loss" class="block text-xs font-semibold text-slate-700">Cắt lỗ</label>
                <div class="flex text-xs font-semibold bg-slate-200 rounded-md p-0.5">
                  <button @click="strategyStore.stopLossUnit = 'pnl'" :class="strategyStore.stopLossUnit === 'pnl' ? 'bg-white text-slate-800 shadow-sm rounded' : 'text-slate-500'" class="px-2 py-0.5">USDT</button>
                  <button @click="strategyStore.stopLossUnit = 'percentage'" :class="strategyStore.stopLossUnit === 'percentage' ? 'bg-white text-slate-800 shadow-sm rounded' : 'text-slate-500'" class="px-2 py-0.5">%</button>
                </div>
              </div>
              <div class="relative">
                <input v-model.number="strategyStore.stopLoss" type="number" id="stop-loss" placeholder="Tùy chọn" class="w-full pl-3 pr-12 py-2 border border-slate-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500"/>
                <span class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-slate-500 text-sm">{{ strategyStore.stopLossUnit === 'pnl' ? 'USDT' : '%' }}</span>
              </div>
            </div>
            <!-- Chốt lời -->
            <div>
              <div class="flex justify-between items-center mb-1">
                <label for="take-profit" class="block text-xs font-semibold text-slate-700">Chốt lời</label>
                <div class="flex text-xs font-semibold bg-slate-200 rounded-md p-0.5">
                  <button @click="strategyStore.takeProfitUnit = 'pnl'" :class="strategyStore.takeProfitUnit === 'pnl' ? 'bg-white text-slate-800 shadow-sm rounded' : 'text-slate-500'" class="px-2 py-0.5">USDT</button>
                  <button @click="strategyStore.takeProfitUnit = 'percentage'" :class="strategyStore.takeProfitUnit === 'percentage' ? 'bg-white text-slate-800 shadow-sm rounded' : 'text-slate-500'" class="px-2 py-0.5">%</button>
                </div>
              </div>
              <div class="relative">
                <input v-model.number="strategyStore.takeProfit" type="number" id="take-profit" placeholder="Tùy chọn" class="w-full pl-3 pr-12 py-2 border border-slate-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500"/>
                <span class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-slate-500 text-sm">{{ strategyStore.takeProfitUnit === 'pnl' ? 'USDT' : '%' }}</span>
              </div>
            </div>
          </div>
      </div>
      ...
      <div class="relative opacity-60 cursor-not-allowed bg-white border border-slate-200 rounded-lg p-4 space-y-3">
        <div class="flex items-center justify-between">
         ...
         <div class="grid grid-cols-2 gap-3">
            <div class="relative">
              <label class="block text-xs font-semibold text-slate-700 mb-1">Trailing Stop (%)</label>
              <input type="number" disabled class="w-full pl-3 pr-4 py-2 border border-slate-300 rounded-md text-sm bg-slate-100"/>
            </div>
            <div class="relative">
              <label class="block text-xs font-semibold text-slate-700 mb-1">Khối lượng lệnh</label>
              <input type="text" value="100% vốn" disabled class="w-full pl-3 pr-4 py-2 border border-slate-300 rounded-md text-sm bg-slate-100"/>
            </div>
          </div>
    ...
    <div id="results-drawer-wrapper" class="absolute bottom-0 left-0 right-0 h-1/3 transform translate-y-full transition-transform duration-300 ease-in-out" :class="{ 'translate-y-0': strategyStore.isShowingResults || isLoading }">
      <div v-if="!strategyStore.isShowingResults && !isLoading && !error" id="results-placeholder" class="h-full bg-white rounded-t-lg border-t border-x border-slate-200 flex items-center justify-center">
      ...
      <div v-if="strategyStore.isShowingResults && strategyStore.backtestResults" id="results-container" class="h-full bg-white rounded-t-lg border-t border-x border-slate-200 flex flex-col">
        <!-- [THAY ĐỔI] Tái cấu trúc header của drawer -->
        <div class="flex justify-between items-center border-b border-slate-200 pr-2">
          <!-- Các Tab điều hướng -->
          <div class="flex">
            <button @click="activeTab = 'overview'" class="tab-button font-semibold px-4 py-3 border-b-2" :class="{ 'active': activeTab === 'overview' }">Tổng quan</button>
            <button @click="activeTab = 'stats'" class="tab-button font-semibold px-4 py-3 border-b-2 border-transparent text-slate-500 hover:text-blue-600" :class="{ 'active': activeTab === 'stats' }">Thống kê</button>
            <button @click="activeTab = 'history'" class="tab-button font-semibold px-4 py-3 border-b-2 border-transparent text-slate-500 hover:text-blue-600" :class="{ 'active': activeTab === 'history' }">Lịch sử Lệnh</button>
          </div>
          <!-- Các nút hành động -->
          <div class="flex items-center space-x-2">
            <button id="view-full-report-btn" @click="viewFullReport" class="text-sm font-semibold text-blue-600 hover:bg-blue-100 px-3 py-1.5 rounded-md flex items-center space-x-1.5">
              <vue-feather type="external-link" size="14"></vue-feather>
              <span>Xem Báo cáo</span>
            </button>
            <button id="reset-view-btn" @click="strategyStore.clearBacktestResults()" class="text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-full p-1" title="Đóng kết quả">
        ...
        <div class="flex-grow p-4 overflow-y-auto">
          <!-- Overview Tab -->
          <div v-if="activeTab === 'overview'" class="tab-content grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="col-span-1 md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4 text-center"> 
              <div class="bg-slate-50 p-3 rounded-lg">
                <p class="text-sm text-slate-500">Tổng Lợi nhuận</p>
                <p class="text-2xl font-bold text-green-600">+{{ strategyStore.backtestResults.overview.total_profit_percent?.toFixed(2) }}%</p>
              </div>
              <div class="bg-slate-50 p-3 rounded-lg">
                <p class="text-sm text-slate-500">Tỷ lệ Thắng</p>
                <p class="text-2xl font-bold text-slate-800">{{ strategyStore.backtestResults.overview.win_rate?.toFixed(2) }}%</p>
              </div>
              <div class="bg-slate-50 p-3 rounded-lg">
                <p class="text-sm text-slate-500">Tổng số Lệnh</p>
                <p class="text-2xl font-bold text-slate-800">{{ strategyStore.backtestResults.overview.total_trades }}</p>
              </div>
              <div class="bg-slate-50 p-3 rounded-lg">
                <p class="text-sm text-slate-500">Lợi nhuận/Rủi ro</p>
                <p class="text-2xl font-bold text-slate-800">{{ strategyStore.backtestResults.overview.profit_factor?.toFixed(2) || 'N/A' }}</p>
              </div>
            </div>
            <!-- NOTE: Equity curve chart is complex, using a placeholder for now -->
            <div class="col-span-1 md:col-span-2 h-32 bg-slate-50 rounded-lg p-2 flex items-center justify-center">
              <p class="text-slate-400 text-sm">Biểu đồ tăng trưởng vốn (Equity Curve) sẽ ở đây</p>  
            </div>
          </div>
          <!-- Detailed Stats Tab -->
          <div v-if="activeTab === 'stats'" class="tab-content">
            <div class="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
              <div class="flex justify-between border-b pb-1"><span class="text-slate-500">Lợi nhuận ròng</span><span class="font-semibold text-green-600">+{{ strategyStore.backtestResults.overview.net_profit?.toFixed(2) || '0.00' }} USDT</span></div>
              <div class="flex justify-between border-b pb-1"><span class="text-slate-500">Lợi nhuận trung bình / lệnh</span><span class="font-semibold text-green-600">+{{ strategyStore.backtestResults.overview.avg_profit_per_trade?.toFixed(2) || '0.00' }} USDT</span></div>
              <div class="flex justify-between border-b pb-1"><span class="text-slate-500">Chuỗi thắng dài nhất</span><span class="font-semibold text-slate-800">{{ strategyStore.backtestResults.overview.longest_win_streak || 0 }}</span></div>
              <div class="flex justify-between border-b pb-1"><span class="text-slate-500">Lỗ trung bình / lệnh</span><span class="font-semibold text-red-600">{{ strategyStore.backtestResults.overview.avg_loss_per_trade?.toFixed(2) || '0.00' }} USDT</span></div>
              <div class="flex justify-between border-b pb-1"><span class="text-slate-500">Chuỗi thua dài nhất</span><span class="font-semibold text-slate-800">{{ strategyStore.backtestResults.overview.longest_loss_streak || 0 }}</span></div>
              <div class="flex justify-between border-b pb-1 text-slate-400"><span class="flex items-center"><vue-feather type="lock" size="12" class="mr-2"></vue-feather>Mức sụt giảm vốn tối đa</span><span class="font-semibold">-12.5%</span></div>
              <div class="flex justify-between border-b pb-1 text-slate-400"><span class="flex items-center"><vue-feather type="lock" size="12" class="mr-2"></vue-feather>Tỷ lệ Sharpe</span><span class="font-semibold">1.8</span></div>
              <div class="col-span-2 text-center mt-2"><a href="#" class="text-blue-600 font-semibold text-xs">Nâng cấp để xem tất cả chỉ số rủi ro</a></div>
            </div>
          </div>
          <!-- History Tab -->
          <div v-if="activeTab === 'history'" class="tab-content">
             <div class="overflow-auto h-full">
                <p v-if="!strategyStore.backtestResults.trade_history || strategyStore.backtestResults.trade_history.length === 0" class="text-center text-slate-400 text-sm pt-4">Không có giao dịch nào được thực hiện.</p>
                <table class="w-full text-sm text-left">
                  ...
                  <tbody>
                    <tr v-for="(trade, index) in strategyStore.backtestResults.trade_history" :key="index" class="border-b" :class="index % 2 === 0 ? 'bg-white' : 'bg-slate-50'">
                      <td class="px-4 py-2">{{ new Date(trade.entry_date).toLocaleString() }}</td>
                      <td class="px-4 py-2"><span class="font-semibold" :class="trade.type === 'MUA' ? 'text-green-600' : 'text-red-600'">{{ trade.type }}</span></td>
                      <td class="px-4 py-2">{{ trade.entry_price?.toFixed(2) }}</td>
                      <td class="px-4 py-2">{{ trade.exit_price?.toFixed(2) }}</td>
                      <td class="px-4 py-2 font-semibold" :class="trade.pnl >= 0 ? 'text-green-600' : 'text-red-600'">{{ trade.pnl?.toFixed(2) }}%</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
const availableIndicators = ref([
    { type: INDICATOR_TYPES.RSI, name: 'RSI', description: 'Relative Strength Index', icon: 'bar-chart-2', color: 'blue' },
    ...
    { type: INDICATOR_TYPES.BB, name: 'Bollinger Bands', description: 'Dải bollinger', icon: 'minimize-2', color: 'cyan' },
    { type: INDICATOR_TYPES.STOCHASTIC, name: 'Stochastic', description: 'Chỉ báo dao động', icon: 'shuffle', color: 'pink' },
    { type: INDICATOR_TYPES.ATR, name: 'ATR', description: 'Average True Range', icon: 'activity', color: 'orange' },
    { type: INDICATOR_TYPES.VOLUME, name: 'Volume', description: 'Khối lượng giao dịch', icon: 'bar-chart', color: 'green' },
    { type: INDICATOR_TYPES.AO, name: 'Awesome Oscillator', description: 'Chỉ báo AO', icon: 'hash', color: 'red' },
    { type: INDICATOR_TYPES.WILLIAMS_R, name: 'Williams %R', description: 'Phần trăm Williams', icon: 'percent', color: 'teal' },
    { type: INDICATOR_TYPES.OBV, name: 'On-Balance Volume', description: 'Chỉ báo OBV', icon: 'arrow-up-circle', color: 'gray' },
]);
...
watch(() => strategyStore.stopLoss, (newValue) => {
  console.log(`[UI] Giá trị Cắt lỗ (Stop Loss) đã thay đổi thành: ${newValue}`);
});

watch(() => strategyStore.takeProfit, (newValue) => {
...
const highestFilterTimeframeRank = computed(() => {
  const allFilters = strategyStore.filters;
  ...
  const maxRank = Math.max(...allFilters.map(condition => {
...
watch(() => strategyStore.timeframe, (newMainTimeframeValue) => {
  ...
  strategyStore.filters.forEach(condition => {
    ...
    if (filterTimeframe && filterTimeframe.rank < mainRank) {
      ...
      console.warn(`[LOGIC] Timeframe bộ lọc '${condition.type}' (${filterTimeframe.label}) không hợp lệ. Tự động cập nhật thành ${mainTimeframe.label}.`);
      condition.timeframe = newMainTimeframeValue;
      toast.info(`Timeframe của bộ lọc ${condition.type} đã được cập nhật thành ${mainTimeframe.label} để đảm bảo tính hợp lệ.`);
    }
  });
});
...
onMounted(async () => {
  ...
  // If we are editing an existing strategy, load it and do nothing else.
  if (isEditMode.value) {
    await strategyStore.loadStrategy(strategyId.value);
    return;
  }

  // If we are creating a new strategy AND a template is specified in the URL
  if (templateIdFromQuery) {
    const selectedTemplate = availableTemplates.value.find(t => t.id === templateIdFromQuery);
    if (selectedTemplate) {
      strategyStore.applyTemplate(selectedTemplate);
    } else {
      // Template in URL is invalid, start with a blank slate.
      strategyStore.resetStrategy();
    }
  } else {
    // If creating a new strategy without a template, just load/reset.
    await strategyStore.loadStrategy(null);
  }
...
async function runBacktest() {
  ...
  try {
    const config = { pair: strategyStore.pair, timeframe: strategyStore.timeframe };

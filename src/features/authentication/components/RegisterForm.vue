<template>
  <div class="max-w-md w-full bg-white p-8 rounded-2xl shadow-lg space-y-6">
    ...
    <div class="space-y-4">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          ...
          <input v-model="form.fullName" @blur="v$.fullName.$touch" id="fullName" name="fullName" type="text" autocomplete="name" class="appearance-none rounded-lg relative block w-full px-3 py-3 border placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-accent-primary focus:border-accent-primary focus:z-10 sm:text-sm" :class="{ 'border-red-500': v$.fullName.$error }" placeholder="Họ và tên">
          <p v-if="v$.fullName.$error" class="text-red-500 text-xs mt-1">{{ v$.fullName.$errors[0].$message }}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <input v-model="form.password" @blur="v$.password.$touch" id="password" name="password" type="password" autocomplete="new-password" class="appearance-none rounded-lg relative block w-full px-3 py-3 border placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-accent-primary focus:border-accent-primary focus:z-10 sm:text-sm" :class="{ 'border-red-500': v$.password.$error }" placeholder="Mật khẩu">
          ...
          <div v-if="form.password" class="mt-2">
            <div class="h-1 w-full bg-gray-200 rounded">
              <div class="h-1 rounded" :class="passwordStrength.class" :style="{ width: passwordStrength.width }"></div>
            </div>
        ...
        <div>
          ...
          <input v-model="form.confirmPassword" @blur="v$.confirmPassword.$touch" id="confirmPassword" name="confirmPassword" type="password" autocomplete="new-password" class="appearance-none rounded-lg relative block w-full px-3 py-3 border placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-accent-primary focus:border-accent-primary focus:z-10 sm:text-sm" :class="{ 'border-red-500': v$.confirmPassword.$error }" placeholder="Xác nhận mật khẩu">
          <p v-if="v$.confirmPassword.$error" class="text-red-500 text-xs mt-1">{{ v$.confirmPassword.$errors[0].$message }}</p>
        </div>
        <div>
    ...
    <p class="text-center text-sm text-gray-600">
      Đã có tài khoản?
      <router-link :to="{ name: 'Login' }" class="font-medium text-accent-primary hover:text-accent-secondary">
        Đăng nhập
      </router-link>
    </p>
  </div>
...
window.handleGoogleRegisterResponse = async (response) => {
  ...
  try {
    ...
    apiError.value = error.response?.data?.detail || 'Đăng ký với Google thất bại. Vui lòng thử lại.';
  }
};
...
const initializeGsi = (retryCount = 0) => {
  if (typeof google !== 'undefined') {
    ...
    google.accounts.id.renderButton(
      ...
      { theme: 'outline', size: 'large', type: 'standard', text: 'signup_with', width: '334' }
    );
    return;
  }

  if (retryCount >= 10) {
    const errorMsg = "Không thể tải dịch vụ đăng ký Google. Vui lòng kiểm tra kết nối mạng và thử lại.";
...
const passwordStrength = computed(() => {
    ...
    switch (strength) {
      ...
      case 2:
        return { width: '50%', class: 'bg-orange-500', text: 'Mật khẩu trung bình', colorClass: 'text-orange-500' };
      case 3:
        return { width: '75%', class: 'bg-yellow-500', text: 'Mật khẩu khá', colorClass: 'text-yellow-500' };
      case 4:
        return { width: '100%', class: 'bg-green-500', text: 'Mật khẩu mạnh', colorClass: 'text-green-500' };
      default:

...
window.handleGoogleCredentialResponse = async (response) => {
  apiError.value = null
  console.log('✅ [GOOGLE LOGIN] Nhận được credential từ Google:', response)
  try {
    ...
    apiError.value =
      error.response?.data?.detail || 'Xác thực với Google thất bại. Vui lòng thử lại.'
  }
}
...
const initializeGsi = (retryCount = 0) => {
  ...
  if (retryCount >= 10) {
    const errorMsg =
      'Không thể tải dịch vụ đăng nhập Google. Vui lòng kiểm tra kết nối mạng và thử lại.'
